...
function msgValidator(req, res, next) {
    const { msg } = req.body
    const { msg_name, msg_date, msg_content } = msg
    ...
    if (!msg_name || !msg_date || !msg_content) {
        ...
        if (typeof msg_name === 'string') {
            if (!_checkMinLength(msg_name, 3)) {
                validationErrors.push(`"msg_name" length must be greater then 3 chars inclusive`)
            }
            if (!_checkMaxLength(msg_name, 10)) {
                validationErrors.push(`"msg_name" length must be shorter then 10 chars inclusive`)
            }
            if (_checkSpaces(msg_name)) {
                validationErrors.push(`"msg_name" must not have trailing or leading spaces`)
            }
        } else {
        ...
        if (!_checkMinLength(msg_content, 15)) {
            validationErrors.push(`"msg_content" length must be greater then 15 chars inclusive`)
        }

        if (validationErrors.length === 0) {
            next()
        } else {
            const errorsStr = validationErrors.map(err => {
